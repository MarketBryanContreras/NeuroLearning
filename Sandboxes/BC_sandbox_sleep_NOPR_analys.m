...
for iS=1%:length(inhib_dir.name)
    ...
    cfg_filt_sg.order = 4; %type filter order
    ...
    %% Obtaining theta pahse and gamma power for the periods power
    % Restrict the filtered csc to the IV of SWS and REM
    theta_awake= restrict(theta_csc,iv_awake);
    %theta_awake_phi=angle(hilbert(theta_awake.data(1,:)));
    theta_sws= restrict(theta_csc,iv_sws);
    %theta_sws_phi=angle(hilbert(theta_sws.data(1,:)));
    theta_rem= restrict(theta_csc,iv_rem);
    %theta_rem_phi=angle(hilbert(theta_rem.data(1,:)));
