...
data_dir = 'C:\Users\bcont\Williams Lab Dropbox\Williams Lab Team Folder\Bryan_DropBox\ChRna2_sleep\Incoming\BC1743_sampling_light_pulses'

ft_dir = 'C:\Users\bcont\Documents\Git\fieldtrip';
...
pdiff = diff(pulse_t);
pulse_start_time = pulse_t([0;find(pdiff > 5)]+1); % find all the pulses with a 5s gap before the previous pulse. Also include the first pulse as a 0 since we add 1 to correct for the diff. 
...
pulse_fast = pulse_start_time(p_type < 1); % start of each pulse train with a pulse duration < 1; In the future you can you different cables and NLX TTL ports to have different event codes. This is just a hack for now. 
pulse_long = pulse_start_time(p_type >= 1); 


% correct time to start at 0
pulse_fast = pulse_fast - start_t_og;
pulse_long = pulse_long - start_t_og; 
pulse_start_time = pulse_start_time - start_t_og; 
...
%% get an event triggered average (ETA) of the LFP 

win_s = .5; % window size in time
ETA = [];
for ii = length(pulse_fast):-1:1
    
    ETA(ii,:) = csc.data(1,nearest_idx(pulse_fast(ii) - win_s, csc.tvec):  nearest_idx(pulse_fast(ii) + win_s, csc.tvec));
